const app = {
    ...
    edit: false,
    get: async function () {
        let resp = await fetch(`${this.url}infringement_types/index`);
        ...
        document.getElementById('data-infringement_types').innerHTML = html;
    },
    getOne: async function (id) {
        try {
            let res = await fetch(`${this.url}infringement_types/show/${id}`);
    ...
    create:async function(form){
        try {
            let res = await fetch(`${this.url}infringement_types/store`, {
                method:'POST',
    ...
    delete: async function (id) {
        try {
            let res  = await fetch(`${this.url}infringement_types/destroy/${id}`);
            let data = await res.json();
            if(data.status === 200){
                toastr.success('', data.message, {
                    closeButton:true
    ...
    update: async function (form, id) {
        try {
            let res = await fetch(`${this.url}infringement_types/edit/${id}`, {
                method:'POST',
                body:new FormData(form)
            });
            let data = await res.json();
...
$(document).ready(async function () {
    ...
    document.getElementById('btn-create').onclick = function(){
        $('.modal #form').trigger('reset');
        $('.modal').modal('toggle');
        $('.modal').find('.modal-title').text('Crear falta');
        val.limpiar();
        val.validaciones();
    }
    ...
    $(document).on('click', '.delete', function () {
        ...
        Swal.fire({
            ...
            if (result.value) {
              Swal.fire("Eliminado!", "El tipo de falta ha sido eliminada.", "success");
