...
class Committee extends Model{
    ...
    {
        try {
            $query = $this->db->connect()->prepare('UPDATE committees SET record_number=:record_number,date=:date, assistants=:assistants, subdirector_name=:subdirector_name, qourum=:qourum,start_hour=:start_hour, end_hour=:end_hour, place=:place, formation_center=:formation_center,updated_at=:updated_at WHERE id=:id');
            if ($query->execute([
                'record_number' => $data['record_number'],
                'date'=>$data['date'],
                'assistants'=>$data['assistants'],
